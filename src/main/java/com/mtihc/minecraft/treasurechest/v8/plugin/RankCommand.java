...
public class RankCommand extends SimpleCommand {
    ...
    public void list(CommandSender sender, String[] args) throws CommandException {
        ...
        StringBuilder addedRanks = new StringBuilder();
        StringBuilder availableRanks = new StringBuilder();
        for (String rank : ranks) {
            if (!tranks.contains(rank.toLowerCase())) {
                availableRanks.append(", ").append(rank);
            } else {
                addedRanks.append(", ").append(rank);
            }
        }
        if (addedRanks.length() == 0) {
            addedRanks = new StringBuilder("(none)");
        } else {
            addedRanks = new StringBuilder(addedRanks.substring(2));//remove first comma and space
        }
        if (availableRanks.length() == 0) {
            availableRanks = new StringBuilder("(none)");
        } else {
            availableRanks = new StringBuilder(availableRanks.substring(2));//remove first comma and space
        }
