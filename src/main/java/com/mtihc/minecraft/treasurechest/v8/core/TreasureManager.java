...
import com.mtihc.minecraft.treasurechest.v8.events.*;
...
public class TreasureManager extends TreasureDataFacade {
    ...
    public void openTreasureInventory(Player player, ITreasureChest tchest) {
        ...
        //
            ...
            long time = whenHasPlayerFound(player, loc);
            ...
            if (time != 0 && !hasForgotten(time, tchest.getForgetTime())) {
                ...
                rememberPlayerFound(player, loc);
    ...
    private boolean checkRank(Player player, ITreasureChest tchest) {
        ...
        StringBuilder rankPerms = new StringBuilder();
        ...
        for (String rank : ranks) {
            ...
            rankPerms.append(", ").append(perm);
        }
        if (rankPerms.length() == 0) {
            return true;
        } else {
            if (player.hasPermission(Permission.SET.getNode())) {
                rankPerms = new StringBuilder(rankPerms.substring(2));//remove comma and string
    ...
    private ItemStack[] getRandomizedInventory(ItemStack[] inventory, int randomAmount) {
        ...
        System.arraycopy(inventory, 0, result, 0, inventory.length);
    ...
    protected void toInventory(ItemStack[] contents, int random, Inventory inventory) {
        ...
        if (sizeTarget < size) {
            ...
            System.arraycopy(tchest, 0, result, 0, tchest.length);
        }
    ...
    abstract static class TreasureInventory implements Runnable {
