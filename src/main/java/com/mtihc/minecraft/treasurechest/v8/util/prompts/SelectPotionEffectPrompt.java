...
public abstract class SelectPotionEffectPrompt extends ValidatingPrompt {
    ...
    public String getPromptText(ConversationContext context) {
        ...
        for (PotionEffectType type : types) {
            ...
            list.append(", ").append(type.getName().toLowerCase());
        }
        if (list.length() > 0) {
            list = new StringBuilder(list.substring(2));
        } else {
            list = new StringBuilder(ChatColor.RED + "There were no potion effects found.");
        }
